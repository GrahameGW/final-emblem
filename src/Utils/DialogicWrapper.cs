...
{
    // From https://github.com/QueenOfSquiggles/SquigglesDialogicSharpWrapper
    public class DialogicWrapper : IDisposable
    {
        //
        ...
        public DialogicWrapper(Node node)
        {
            ...
            _signalRefs = new() {
                ...
                {"timeline_ended", Callable.From(() => { 
                    GD.Print("TimelineEnded"); 
                ...
                {"signal_event_bool", Callable.From<string>((arg) => {
                    bool res = arg.ToLower() == "true" || arg.ToUpper() == "T";
                    SignalBoolEvent?.Invoke(res);
                    })
                }
            };
            _names = new() {
                ...
                {"paused", "paused"},
                {"start", "start"},
                {"signal_event_bool", "signal_event" },
            };
        ...
        public event Action<bool> SignalBoolEvent;
